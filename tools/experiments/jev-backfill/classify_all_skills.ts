/** Authorized classification only: 1100 additional reservations, no memory/skill writes. */
...
const out=resolve('artifacts/jev-backfill/full-skill-classification-v1');mkdirSync(out,{recursive:true,mode:0o700});
...
try {
 ...
 const valid:any[]=[],excluded:any[]=[];const checked=new Map<string,boolean>();
 for(const r of rows){
  if(!checked.has(r.path))checked.set(r.path,existsSync(r.path)&&hash(readFileSync(r.path,'utf8'))===r.sha256);
  if(!checked.get(r.path)){excluded.push({path:r.path,reason:'source-changed',start:r.start_offset});continue;}
 ...
 writeFileSync(resolve(out,'plan.json'),JSON.stringify({version:1,authorizedAdditionalCalls:1100,records:rows.length,eligibleChunks:valid.length,batches:batches.length,exclusions:excluded,classificationOnly:true,scope:'current active/archive skill bodies and text supports; historical revisions inventoried separately, not classified by this run'},null,2),{mode:0o600});
 ...
 for(const b of batches){
  const id=hash('skill-v1:jev-1.13.0:'+JSON.stringify(b));const resultPath=resolve(out,id+'.json');
